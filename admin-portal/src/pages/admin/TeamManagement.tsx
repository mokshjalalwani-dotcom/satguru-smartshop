...
import { Users, UserPlus, Clock, CheckCircle2, TrendingUp, MoreVertical, Shield } from "lucide-react";
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-bold hover:shadow-[0_0_20px_rgba(99,102,241,0.4)] transition-all">
          <UserPlus size={18} />
          Add Member
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-xcard border border-white/5 p-6 rounded-2xl flex items-center gap-4 relative overflow-hidden group">
          ...
          <div><p className="text-sm text-xtext-secondary">Avg AI Score</p><p className="text-2xl font-bold">88.2</p></div>
        </div>
      </div>
        ...
        {staff.map(employee => (
          <div key={employee.id} className="bg-xcard border border-white/5 rounded-3xl p-6 hover:border-indigo-500/30 transition-all hover:-translate-y-1 relative group overflow-hidden">
            <div className="flex justify-between items-start mb-6">
              ...
              <button className="text-xtext-secondary hover:text-white p-1 rounded-lg hover:bg-white/10 transition-colors">
            ...
            <div className="space-y-3 mb-6 relative z-10">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className={`font-bold px-2 py-0.5 rounded-md ${employee.role === 'Admin' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'bg-white/5 text-white/80 border border-white/10'}`}>
                  {employee.role}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
            ...
            <div className="border-t border-white/5 pt-4">
              ...
              <div className="w-full h-1.5 bg-background rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full ${employee.aiScore > 90 ? 'bg-emerald-400' : employee.aiScore > 80 ? 'bg-cyan-400' : 'bg-amber-400'}`}
                  style={{ width: `${employee.aiScore}%` }}
                />
              </div>
            </div>
          </div>
